...
use web_sys::js_sys::{Reflect, Array};
...
pub fn PageHome() -> Html {
    ...
    use_effect(|| {
        ...
        let init_date_pickers = Reflect::get(&flowbite, &JsValue::from_str("initDatepickers")).unwrap();
        let arr = Array::new();
        // let function_1 = function.("initDatepickers").expect("Flowbite.initDatepickers");
        // web_sys::console::log_1(&initDatepickers);
        Reflect::apply(init_date_pickers.unchecked_ref(), &JsValue::from_str(""), &arr)
          .expect("No error");
    });
